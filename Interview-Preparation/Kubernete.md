...
# 🔗 Relationships Between Kubernetes Objects

Understanding how Kubernetes objects relate to each other helps manage your applications more effectively:

- 🧩 **Pod** → Manages one or more **containers** (e.g., Tomcat, Ubuntu).
- 🔁 **ReplicaSet** → Ensures the desired number of **pod replicas** are running at all times.
- 🌐 **Service** → Exposes **pods** to the **outside world** (or internal traffic) and handles load balancing.
- ⚙️ **ConfigMap** → Provides **configuration data** to pods (non-sensitive).
- 🔐 **Secret** → Provides **sensitive data** (like API keys, passwords) securely to pods.
- 🗄️ **Volume** → Provides **persistent storage** to containers in a pod.
- 🚀 **Deployment** → Manages **ReplicaSets**, enabling **rolling updates** and **rollbacks**.
- ✅ **Job** → Runs pods to **completion** for batch/one-time tasks.
- 📦 **DaemonSet** → Ensures a **pod copy** runs on **all (or selected)** nodes.
- 🗂️ **Namespace** → Organizes Kubernetes resources into **logical groups**.
...
| 🏷️ **Field**    | 📝 **Description**                                              |
|------------------|----------------------------------------------------------------|
| 🔢 `replicas`    | Number of desired pod instances (e.g., `2/2`)                  |
| 🚀 `startup`     | Defines how the container should start                         |
| 🖼️ `image`       | Base container image (e.g., `tomcat`, `ubuntu`)                |
| 🧷 `detached`    | Run container in the background (default behavior)             |
